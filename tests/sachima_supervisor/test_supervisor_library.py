...
def test_expected_pin_matches_packaged_distribution_pin() -> None:
    assert EXPECTED_AGENT_RUN_SUPERVISOR_VERSION == "0.1.6"
...
def test_importable_and_exact_pin_reports_ready() -> None:
    ...
    status = check_supervisor_library_pin(
        ...
        version_probe=_counting_version(version_calls, "0.1.6"),
    )
    ...
    assert status.expected_version == "0.1.6"
    assert status.observed_version == "0.1.6"
...
@pytest.mark.parametrize(
    "observed",
    [None, 7, "", "0.1.6 with secret tok" + "en detail", "0.1.6\nextra-line"],
)
...
@pytest.mark.parametrize("expected", ["", "not a version!!", "0.1.6\n", None, 7])

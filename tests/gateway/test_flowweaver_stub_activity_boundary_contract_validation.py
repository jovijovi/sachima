"""RED gate tests for FlowWeaver Phase 26 stub Activity boundary contract validation."""
...
MODULE_SOURCE = ROOT / "gateway" / "flowweaver_stub_activity_boundary_contract_validation.py"
PLAN_DOC = ROOT / "docs" / "plans" / "2026-05-09-flowweaver-phase26-stub-activity-boundary-contract-validation.md"
DEV_LOG = ROOT / "docs" / "dev_log" / "2026-05-09-flowweaver-phase26-stub-activity-boundary-contract-validation.md"
RUNBOOK = ROOT / "docs" / "runbooks" / "flowweaver-stub-activity-boundary-contract-validation.md"
...
EXPECTED_ACTIVITY_INTERFACE_NAMES = ["validate_claim_check_ref", "execute_agent_turn", "deliver_artifact"]
...
PRIVATE_CHAT_ID = "oc_" + "phase26_private_chat"
PRIVATE_USER_ID = "ou_" + "phase26_private_user"
PRIVATE_MESSAGE_ID = "om_" + "phase26_private_message"
RAW_PROMPT_VALUE = "raw prompt phase26 private value"
RAW_TOOL_VALUE = "raw " + "tool output phase26 private value"
CARD_JSON_VALUE = '{"type":"card_json","body":"phase26"}'
MEDIA_PATH_VALUE = "/tmp/phase26-private.png"
CALLBACK_VALUE = "callback payload phase26 private value"
RAW_EXCEPTION_VALUE = "RuntimeError: raw phase26 exception value"
SENSITIVE_SENTINEL = "unsafe-" + "material" + "-phase26"
BEARER_VALUE = "Bearer " + "phase26-private"
OPENAI_KEY_VALUE = "sk-" + "phase26-private"
...
def execution_request() -> dict[str, object]:
    return {
        ...
        "transaction_id": "runtime_tx_phase26_1",
        "workflow_id": "runtime_tx_phase26_1",
...
def phase25_error_report() -> dict[str, object]:
    return build_flowweaver_stub_activity_boundary_contract_report(
        validation_contract_descriptor={"bad": RAW_PROMPT_VALUE},
        validation_report=phase24_report(),
    )
...
def expected_error_report(error_code: str) -> dict[str, object]:
    return {
        ...
        "phase": "phase26",
        "ok": False,
        "operation": "validate_flowweaver_stub_activity_boundary_contract",
...
def expected_success_report() -> dict[str, object]:
    return {
        ...
        "operation": "validate_flowweaver_stub_activity_boundary_contract",
        "phase25_verdict": FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_SUCCESS_VERDICT,
        "activity_interface_names": EXPECTED_ACTIVITY_INTERFACE_NAMES,
...
def test_phase26_exposes_pure_synchronous_keyword_only_entrypoints() -> None:
    signature = inspect.signature(build_flowweaver_stub_activity_boundary_contract_validation_report)
    assert list(signature.parameters) == ["boundary_contract_descriptor", "boundary_contract_report"]
    assert all(parameter.kind is inspect.Parameter.KEYWORD_ONLY for parameter in signature.parameters.values())
    assert signature.return_annotation == "dict[str, object]"
    assert not inspect.iscoroutinefunction(build_flowweaver_stub_activity_boundary_contract_validation_report)
...
def test_phase26_contract_descriptor_freezes_validation_scope_and_success_verdict() -> None:
    contract = describe_flowweaver_stub_activity_boundary_contract_validation_contract()

    assert list(contract) == EXPECTED_CONTRACT_FIELDS
    assert contract["type"] == FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_VALIDATION_CONTRACT_TYPE
    assert contract["version"] == FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_VALIDATION_VERSION
    assert contract["phase"] == "phase26"
    assert contract["verdict"] == FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_VALIDATION_SUCCESS_VERDICT
    assert contract["verdict"] == "ready_for_stub_activity_implementation_design"
    assert contract["scope"] == "stub_activity_boundary_contract_validation"
    assert contract["consumes_contract"] == FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_TYPE
    assert contract["consumes_report"] == FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_REPORT_TYPE
...
def test_phase26_happy_path_builds_safe_validation_report_from_phase25_boundary_report() -> None:
    report = build_flowweaver_stub_activity_boundary_contract_validation_report(
...
def test_phase26_validator_returns_sanitized_deep_copy() -> None:
    report = expected_success_report()
    safe = validate_flowweaver_stub_activity_boundary_contract_validation_report(report)
    safe["activity_interface_names"].append("mutated")  # type: ignore[index, union-attr]
    safe["validation_summary"]["runtime_side_effects"] = "mutated"  # type: ignore[index]
    safe["checks"]["phase25_contract_valid"] = False  # type: ignore[index]

    assert report == expected_success_report()


def test_phase26_rejects_invalid_phase25_contract_with_sanitized_error_report() -> None:
    result = build_flowweaver_stub_activity_boundary_contract_validation_report(
        boundary_contract_descriptor={"type": RAW_PROMPT_VALUE},
        boundary_contract_report=phase25_report(),
    )

    assert result == expected_error_report("invalid_phase25_boundary_contract")
    assert_no_raw_values(result)


def test_phase26_rejects_invalid_phase25_report_with_sanitized_error_report() -> None:
    report = copy.deepcopy(phase25_report())
    report["activity_interfaces"][0]["error_codes"][0] = RAW_PROMPT_VALUE  # type: ignore[index]

    result = build_flowweaver_stub_activity_boundary_contract_validation_report(
        boundary_contract_descriptor=describe_flowweaver_stub_activity_boundary_contract(),
        boundary_contract_report=report,
    )

    assert result == expected_error_report("invalid_phase25_boundary_contract_report")
    assert_no_raw_values(result)


def test_phase26_rejects_phase25_error_report_with_sanitized_error_report() -> None:
    result = build_flowweaver_stub_activity_boundary_contract_validation_report(
        boundary_contract_descriptor=describe_flowweaver_stub_activity_boundary_contract(),
        boundary_contract_report=phase25_error_report(),
    )
...
def test_phase26_validator_rejects_report_field_mutations(mode: str) -> None:
    report = rebuilt_with_fields(expected_success_report(), mutate_list(EXPECTED_REPORT_FIELDS, mode))

    with pytest.raises(ValueError, match="invalid_stub_activity_boundary_contract_validation_report"):
        validate_flowweaver_stub_activity_boundary_contract_validation_report(report)


@pytest.mark.parametrize("mode", ["missing", "extra", "reordered", "duplicated", "bogus"])
def test_phase26_contract_descriptor_rejects_report_field_list_mutations(mode: str) -> None:
    contract = describe_flowweaver_stub_activity_boundary_contract_validation_contract()
    contract["report_fields"] = mutate_list(EXPECTED_REPORT_FIELDS, mode)

    with pytest.raises(ValueError, match="invalid_stub_activity_boundary_contract_validation_contract"):
        validate_flowweaver_stub_activity_boundary_contract_validation_contract_descriptor_for_test(contract)


@pytest.mark.parametrize("mode", ["missing", "extra", "reordered", "duplicated", "bogus"])
def test_phase26_validator_rejects_activity_interface_name_mutations(mode: str) -> None:
    report = expected_success_report()
    report["activity_interface_names"] = mutate_list(EXPECTED_ACTIVITY_INTERFACE_NAMES, mode)
...
def test_phase26_validator_rejects_check_mutations(key: str) -> None:
    report = expected_success_report()
...
@pytest.mark.parametrize(
    ("path", "replacement"),
    [
        (("type",), SneakyStr(FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_VALIDATION_REPORT_TYPE)),
        (("ok",), 1),
        (("phase25_verdict",), SneakyStr(FLOWWEAVER_STUB_ACTIVITY_BOUNDARY_CONTRACT_SUCCESS_VERDICT)),
        (("activity_interface_names",), SneakyList(EXPECTED_ACTIVITY_INTERFACE_NAMES)),
        (("validation_summary",), SneakyDict(EXPECTED_VALIDATION_SUMMARY)),
        (("validation_summary", "activity_interface_count"), True),
        (("checks",), SneakyDict({key: True for key in EXPECTED_CHECKS})),
        (("side_effects",), SneakyList()),
    ],
)
def test_phase26_validator_rejects_hostile_subclass_and_bool_impersonation_values(
...
def test_phase26_validator_rejects_hostile_top_level_and_nested_keys() -> None:
    report = {SneakyKeyStr(key): value for key, value in expected_success_report().items()}
    with pytest.raises(ValueError, match="invalid_stub_activity_boundary_contract_validation_report"):
        validate_flowweaver_stub_activity_boundary_contract_validation_report(report)

    report = {ExplodingKeyStr(key): value for key, value in expected_error_report("invalid_phase25_boundary_contract").items()}
    with pytest.raises(ValueError, match="invalid_stub_activity_boundary_contract_validation_report"):
        validate_flowweaver_stub_activity_boundary_contract_validation_report(report)

    report = expected_success_report()
    report["validation_summary"] = {
        SneakyKeyStr(key): value for key, value in EXPECTED_VALIDATION_SUMMARY.items()
    }
    with pytest.raises(ValueError, match="invalid_stub_activity_boundary_contract_validation_report"):
        validate_flowweaver_stub_activity_boundary_contract_validation_report(report)
...
def test_phase26_builder_rejects_hostile_phase25_descriptor_and_report_keys() -> None:
    sneaky_descriptor = {
        SneakyKeyStr(key): value for key, value in describe_flowweaver_stub_activity_boundary_contract().items()
    }
    result = build_flowweaver_stub_activity_boundary_contract_validation_report(
        boundary_contract_descriptor=sneaky_descriptor,
        boundary_contract_report=phase25_report(),
    )
    assert result == expected_error_report("invalid_phase25_boundary_contract")

    sneaky_report = {SneakyKeyStr(key): value for key, value in phase25_report().items()}
    result = build_flowweaver_stub_activity_boundary_contract_validation_report(
        boundary_contract_descriptor=describe_flowweaver_stub_activity_boundary_contract(),
        boundary_contract_report=sneaky_report,
    )
    assert result == expected_error_report("invalid_phase25_boundary_contract_report")


def test_phase26_builder_rejects_internally_inconsistent_phase25_descriptor_report_pair() -> None:
    report = phase25_report()
    report["payload_policy"] = copy.deepcopy(describe_flowweaver_stub_activity_boundary_contract()["payload_policy"])
    report["payload_policy"]["canonical_input_kind"] = "message text"  # type: ignore[index]
...
def test_phase26_error_report_validator_accepts_only_sanitized_error_codes() -> None:
    safe = validate_flowweaver_stub_activity_boundary_contract_validation_report(
        expected_error_report("invalid_phase25_boundary_contract")
    )
    assert safe == expected_error_report("invalid_phase25_boundary_contract")

    bad = expected_error_report(RAW_PROMPT_VALUE)
    with pytest.raises(ValueError, match="invalid_stub_activity_boundary_contract_validation_report"):
        validate_flowweaver_stub_activity_boundary_contract_validation_report(bad)


def test_phase26_source_has_no_runtime_or_builder_escape_hatches() -> None:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.ImportFrom):
            ...
            assert module != "gateway.flowweaver_stub_activity_orchestration"
            assert module != "gateway.flowweaver_stub_activity_orchestration_validation"
...
def test_phase26_changed_file_guard_allows_only_boundary_validation_files() -> None:
...
def test_phase26_docs_record_verdict_boundaries_and_verification() -> None:
    for path in (PLAN_DOC, DEV_LOG, RUNBOOK):
        text = path.read_text()
        assert "ready_for_stub_activity_implementation_design" in text
        ...
        assert "build_flowweaver_stub_activity_boundary_contract_report" in text
        assert "orchestrate_flowweaver_stub_activities" in text
        assert "tests/gateway/test_flowweaver_stub_activity_boundary_contract_validation.py" in text


def validate_flowweaver_stub_activity_boundary_contract_validation_contract_descriptor_for_test(
    value: object,
) -> dict[str, object]:
    """Local test copy of the expected P26 descriptor strictness."""

    if type(value) is not dict or list(value) != EXPECTED_CONTRACT_FIELDS:
        raise ValueError("invalid_stub_activity_boundary_contract_validation_contract")

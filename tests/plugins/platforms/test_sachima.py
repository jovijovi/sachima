...
from plugins.platforms.sachima.adapter import (
    SachimaAdapter,
    SachimaPayloadError,
...
def test_sachima_stays_unready_without_explicit_delivery_or_test_mode():
...
def test_plugin_registration_is_default_off_and_uses_dynamic_platform_factory():
...
def test_build_event_from_payload_maps_base64_png_attachment_to_photo_event(monkeypatch):
    ...
    monkeypatch.setattr("plugins.platforms.sachima.adapter.cache_image_from_bytes", fake_cache)
    adapter = SachimaAdapter(PlatformConfig(enabled=True))
...
def test_build_event_from_payload_uses_placeholder_text_for_image_without_text(monkeypatch):
    ...
    monkeypatch.setattr("plugins.platforms.sachima.adapter.cache_image_from_bytes", lambda data, ext: "/tmp/image.png")
    adapter = SachimaAdapter(PlatformConfig(enabled=True))
...
def test_build_event_from_payload_rejects_oversized_base64_before_decoding(monkeypatch):
    ...
    monkeypatch.setattr("plugins.platforms.sachima.adapter.base64.b64decode", fail_decode)
...
def test_build_event_from_payload_accepts_exact_decoded_size_limit(monkeypatch):
    ...
    monkeypatch.setattr("plugins.platforms.sachima.adapter.cache_image_from_bytes", lambda data, ext: "/tmp/exact.png")
...
async def test_handle_webhook_payload_downloads_safe_image_url(monkeypatch):
    ...
    async def fake_cache_url(url, ext):
        assert url == "https://example.com/image.png"
        assert ext == ".png"
        return "/tmp/url-image.png"

    monkeypatch.setattr("plugins.platforms.sachima.adapter.cache_image_from_url", fake_cache_url)
...
async def test_handle_webhook_payload_prefers_base64_when_attachment_also_has_url(monkeypatch):
    ...
    monkeypatch.setattr("plugins.platforms.sachima.adapter.cache_image_from_bytes", fake_cache_bytes)
    monkeypatch.setattr("plugins.platforms.sachima.adapter.cache_image_from_url", fail_cache_url)
    adapter = SachimaAdapter(PlatformConfig(enabled=True))
    adapter.handle_message = AsyncMock()
...
async def test_send_without_send_url_records_local_outbound_message():
    ...
    adapter = SachimaAdapter(PlatformConfig(enabled=True, extra={"local_mode": True}))
...
async def test_send_without_send_url_chunks_long_outbound_message():
    ...
    adapter = SachimaAdapter(PlatformConfig(enabled=True, extra={"local_mode": True, "max_message_length": 80}))

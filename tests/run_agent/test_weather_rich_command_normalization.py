...
def test_weather_helper_command_is_not_rewritten_until_enabled():
    args = {
...
def test_feishu_weather_helper_command_without_format_gets_hermes_json_when_enabled():
    args = {
        "command": f"python3 {WEATHER_HELPER} --lat 30.5728 --lon 104.0668 --period today"
    }

    normalized = _normalize_weather_rich_terminal_args("feishu", "terminal", args, enabled=True)

    assert normalized is not args
    assert normalized["command"].endswith("--format hermes-json")


def test_feishu_weather_helper_command_with_text_format_is_forced_to_hermes_json_when_enabled():
    args = {
...
def test_lark_weather_helper_command_without_format_gets_hermes_json_when_enabled():
    args = {"command": f"python3 {WEATHER_HELPER} --location Chengdu --period today"}

    normalized = _normalize_weather_rich_terminal_args("lark", "terminal", args, enabled=True)
...
def test_feishu_rejects_shell_syntax_and_fake_weather_helper_paths():
    ...
    assert _normalize_weather_rich_terminal_args("feishu", "terminal", shell_args, enabled=True) == shell_args
    assert _normalize_weather_rich_terminal_args("feishu", "terminal", fake_path_args, enabled=True) == fake_path_args

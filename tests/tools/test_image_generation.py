...
class TestRegistryIntegration:
    ...
    def test_schema_exposes_expected_agent_params(self, image_tool):
        """The agent-facing schema exposes prompt, aspect_ratio, image edit inputs,
        and log-only content_summary. Model selection stays user-level config;
        content_summary must not be forwarded to providers."""
        props = image_tool.IMAGE_GENERATE_SCHEMA["parameters"]["properties"]
        assert set(props.keys()) == {
            "prompt", "aspect_ratio", "image_url", "reference_image_urls", "content_summary",
        }

...
current_position: P4 Sachima-side local conformance implemented; agentic-ui/cross-repo conformance pending as a side tail; agent-run-supervisor Sachima local/offline integration implementation added as a default-off, non-Gateway local/offline seam; supervised local Activity design and exec_dry_run implementation are merged; controlled local dry-run evidence/fixtures merged in PR #100 and status closed in PR #101; docs-only durable runtime ownership & controlled local execution design merged in PR #102 (`e49709d6e960b8e11f8e220fa087488132f64f93`); local/offline durable-state preflight implementation merged in PR #107 (`6795da2930324cde1448586e71ff8d80bc6e9ae1`); Phase C controlled local agent execution first slice (exec_controlled one-shot wrapper, pinned-local-acpx provenance gate, atomic pre-launch claim/CAS, read-only Codex primary reviewer role only) merged in PR #114 (`21d1bafc22c6fcde2bb0af6fff6becfb0886cf4f`) per the 2026-06-12 PRD gate and user approval — Codex re-review PASS, PR CI green, post-merge local verification passed, real local smoke NOT run (no pinned local acpx binary on host; prompt materialization deferred; `agent_run_supervisor` absent on this host); Phase D deterministic real local smoke readiness gate is docs-only and prepared by Codex CLI substituting for an interrupted Claude Code architecture/docs pass; fresh-context Codex primary review first BLOCKED two docs inconsistencies, narrow fixes landed, and blocker-only re-review returned PASS / BLOCKERS None; readiness gate merged in PR #117 (`eb7227301d715b40d4eb6628bf32fb800017bd42`); the separately approved Phase D smoke **prerequisites implementation** (pinned local acpx provenance verifier, deterministic prompt materialization fixture/builder plus an explicit `prompt_materializer` seam that defaults to the Phase C `prompt=None` posture, and an `agent_run_supervisor` exact-pin checker) merged in PR #119 (`0c9e4342e2befe1db6ecf5774c51b313c8bb5f5b`) after PR checks and post-merge local verification passed — it ran no smoke and launched no agent, and the host still has no pinned local acpx and no installed `agent_run_supervisor`; persistent sessions, cancellation execution, write-capable roles, Satine/Hermes-profile ACP, controlled AI FLOW execution, live/public, Gateway involvement, and real delivery are not approved
implementation_marker_note: no live / no Gateway / no real delivery / no real local smoke yet / controlled one-shot exec wrapper + smoke prerequisites preparation only
```
...
| Readiness — Phase D deterministic real local smoke | Merged in PR #117; execution BLOCKED | PRD / architecture / user review packet / manifest / dev log under `docs/plans/2026-06-12-agent-run-supervisor-sachima-phase-d-deterministic-real-local-smoke-readiness-*`; Claude Code attempted architecture/docs then hit session-limit / 429; Codex CLI substituted for authoring; Codex primary re-review PASS / BLOCKERS None; PR #117 merge `eb7227301d715b40d4eb6628bf32fb800017bd42` | Defines pinned local `acpx` provisioning, untracked local-only role overlay, prompt materialization scheme, sanitized evidence, replay proof, and Definition of Ready. Runs no smoke, no AGENT, no `acpx`, no Gateway/Feishu/live, no service/runtime, and no production config. Execution remains blocked on missing pinned local `acpx`, absent untracked local role overlay, absent installed/pinned `agent_run_supervisor`, and separate user approval; repo-side prompt materialization was later implemented in PR #119. |
| Implementation — Phase D smoke prerequisites | Merged in PR #119; local/post-merge verification passed; real smoke still blocked | Plan `docs/plans/2026-06-12-agent-run-supervisor-sachima-phase-d-smoke-prerequisites-implementation.md` + manifest + dev log; code `sachima_supervisor/activity_controlled_exec.py` (provenance helper + `prompt_materializer` seam), `sachima_supervisor/smoke_prompt.py`, `sachima_supervisor/supervisor_library.py`; fixture `tests/fixtures/sachima_supervisor/phase_d_smoke_prompt.v1.txt`; tests `tests/sachima_supervisor/` (suite 325 green incl. new provenance/materializer/pin tests and the Codex round-1 review-fix tests); PR #119 merge `0c9e4342e2befe1db6ecf5774c51b313c8bb5f5b`; Codex blocker-only review round 1 BLOCKED two gates (substring version match in the acpx provenance verifier; npx-only basename check at the launch gate) — both fixed with an exact-version-token match and a shared forbidden-runner predicate; blocker-only re-review PASS / BLOCKERS None | Implements the three readiness-gate prerequisite layers with injected fakes only: pinned local acpx provenance verification (injected probe; binary never executed here), deterministic bounded no-leak prompt materialization behind an explicit opt-in seam (default stays Phase C `prompt=None`; failure collapses to `activity_prompt_materialization_failed` with zero supervisor calls; replay never rematerializes), and an `agent_run_supervisor` exact-pin checker (expected `0.0.0`; library deliberately NOT added to dependencies). Does not run or approve a smoke, an AGENT, `acpx`/`npx`, persistent sessions, cancellation, write-capable roles, Satine, controlled AI FLOW, live/Gateway/Feishu, or production config; host provisioning blockers remain |
...
| ROADMAP-NEXT-ARS-CTRL-EXEC-REAL-SMOKE | NEXT_PHASE | Phase D deterministic real local smoke (read-only Codex one-shot) is currently BLOCKED on remaining host/execution prerequisites: (1) no pinned local acpx binary on this host (committed role config truthfully keeps `acpx_binary: null` and the provenance gate fails closed); (2) no untracked local-only role overlay has been generated for a verified binary; (3) the `agent_run_supervisor` library is not installed/pinned on this host (surfaced by the Phase D readiness design pass), so the seam would raise `supervisor_library_unavailable` until provisioned. A **docs-only readiness gate** is now prepared (PRD + architecture/readiness packet + user review packet + manifest + dev log; see references) — Claude Code attempted architecture/docs and hit session-limit / 429, Codex CLI substituted for authoring, fresh-context Codex primary review first BLOCKED two docs consistency issues, narrow fixes landed, and blocker-only re-review returned PASS / BLOCKERS None; PR #117 merged; design only, no smoke run, no approval to execute. | No | Yes | Any real local smoke or real AGENT execution claim | Operator pins a verified local acpx executable (carried in an untracked local-only role overlay, not committed config), the repo-side prompt materialization/prerequisite layer from PR #119 remains in place, the `agent_run_supervisor` library is installed/pinned, and a separately named user approval is given; the smoke then runs with no Gateway/delivery/public ingress and sanitized evidence only | Open — BLOCKED; docs-only readiness gate merged in PR #117 (Codex primary review PASS); the separately approved **prerequisites implementation** (acpx provenance verifier, prompt materialization fixture/builder + opt-in seam, library pin checker) merged in PR #119 (`0c9e4342e2befe1db6ecf5774c51b313c8bb5f5b`) — it satisfies the repo-side prompt-materialization/prerequisite-implementation layer, while the host-side prerequisites (pinned local acpx binary; installed/pinned `agent_run_supervisor`; untracked local role overlay) remain unprovisioned; real smoke execution still requires full Definition of Ready plus separate approval |
...
The Phase D readiness gate is now merged docs-only. Claude Code attempted architecture/docs for the readiness gate and then hit session-limit / 429; Codex CLI substituted for the architecture/docs authoring role. Fresh-context Codex primary review first returned BLOCK on two docs consistency issues; after narrow fixes, blocker-only re-review returned PASS / BLOCKERS None. The subsequent prerequisites-preparation request has also been exercised: the 2026-06-12 user approval ("批准准备 Phase D smoke prerequisites：实现并验证 pinned local acpx provenance、prompt materialization、agent_run_supervisor 安装/固定方案…") produced the prerequisites implementation slice (plan/manifest/dev log under `docs/plans/2026-06-12-agent-run-supervisor-sachima-phase-d-smoke-prerequisites-*`), with Codex blocker-only re-review PASS / BLOCKERS None; PR #119 merged at `0c9e4342e2befe1db6ecf5774c51b313c8bb5f5b` and post-merge local verification passed. The slice ran no smoke and launched no agent.

The next allowed request on this mainline is **host-local Phase D smoke provisioning / DoR verification**, not smoke execution:

```text
1. Separately approved host-local provisioning for Phase D smoke readiness: verify a pinned local acpx,
   create an untracked local-only role overlay, and install/pin agent_run_supervisor — still with no
   Gateway/Feishu/live, no production config write, and no smoke until a later explicit approval.
```

After that review and user approval, a **later** Phase D deterministic real local smoke request remains BLOCKED until the operator provides a verified pinned local `acpx` executable via an untracked local-only role overlay, the `agent_run_supervisor` library is installed/pinned on the host, host provisioning is complete, and the user gives a separate named smoke approval. See `ROADMAP-NEXT-ARS-CTRL-EXEC-REAL-SMOKE`.
...
- The Phase D smoke prerequisites implementation ships verifiers, builders, and checkers exercised with injected fakes only. It does not provision a pinned local acpx binary, does not install `agent_run_supervisor` on any host, does not generate a role overlay, and does not approve or run a smoke; the explicit `prompt_materializer` seam defaults to the Phase C `prompt=None` posture and nothing wires a materializer in by default.

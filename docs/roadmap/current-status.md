...
The current Phase D readiness branch is docs-only. Claude Code attempted architecture/docs for the readiness gate and then hit session-limit / 429; Codex CLI substituted for the architecture/docs authoring role. Fresh-context Codex primary review first returned BLOCK on two docs consistency issues; after narrow fixes, blocker-only re-review returned PASS / BLOCKERS None. The next allowed request on this branch is **PR #117 CI/user review of the docs-only readiness gate**, not smoke execution:

```text
1. PR #117 CI / user review of the Phase D readiness PRD / architecture packet /

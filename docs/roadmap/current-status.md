...
| Architecture authority | `docs/architecture/sachima-global-architecture-agent-run-supervisor-temporal.md` is the authoritative architecture source for the agent-run-supervisor × Temporal integration: it names the components, maps the S0–S5 / P7 milestone labels to the actual components/contracts they produced, and separates target vs current-state vs gaps (notably the still-unbuilt delivery receiver/bridge and its safe-label → concrete-target mapping). Docs-only; it moves no approval. |

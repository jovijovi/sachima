...
latest_ars_remaining_goals_plan: agent-run-supervisor × Sachima remaining-goals plan and implementation roadmap (docs-only planning gate; Claude Code architect draft; Codex CLI blocker-only review PASS; Hermes accepted with WP1a/WP1b split; PR #128 open from `docs/agent-run-supervisor-remaining-goals-plan`; docs-only planning gate; no implementation/execution/additional acpx/live/Gateway/Feishu/production config/real delivery approval)

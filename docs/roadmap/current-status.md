...
latest_phase_e_persistent_sessions_cancellation_design: agent-run-supervisor × Sachima Phase E persistent sessions / cancellation design (docs-only; design only; candidate PR pending — Hermes patches PR fields after creation)
...
current_position: P4 Sachima-side local conformance implemented; agentic-ui/cross-repo conformance pending as a side tail; agent-run-supervisor → Sachima local/offline integration, supervised local Activity, controlled dry-run evidence, durable-state preflight, and Phase C controlled local exec wrapper have merged; Phase D readiness docs merged in PR #117 and repo-side smoke prerequisites merged in PR #119; after separate user approvals, host-local DoR provisioning passed and one deterministic Phase D real local read-only Codex smoke ran through pinned local acpx 0.10.0 with execution pipeline PASS and business verdict PASS. Evidence is stored outside the repo under `/home/ubuntu/workspace/hermes/outputs/sachima/phase-d-real-local-smoke/`. This proves one bounded local read-only smoke only; persistent sessions, cancellation execution, write-capable roles, Satine/Hermes-profile ACP, controlled AI FLOW execution, live/public ingress, Gateway/Feishu involvement, production config writes, and real delivery remain unapproved. A Phase E persistent sessions / cancellation docs-only design packet has been drafted and its candidate PR is pending; the packet designs persistent-session and cancellation semantics only and does not approve implementation or any execution.
...
- agent-run-supervisor Sachima Phase E persistent sessions / cancellation design packet (docs-only; design only; candidate PR pending): `docs/plans/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-sessions-cancellation-design.md` (+ manifest `docs/plans/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-sessions-cancellation-design-manifest.yaml`)
...
| ROADMAP-NEXT-ARS-PHASE-E-SESSIONS-CANCELLATION-DESIGN | NEXT_PHASE | Phase E persistent sessions / cancellation design packet drafted as a docs-only candidate PR (PR number pending). Persistent-session and cancellation implementation/execution remain separately gated; the design recommends the local/offline lifecycle preflight / state-machine slice (injected fakes only) as the first later implementation gate. | No | No | Any persistent-session or cancellation implementation request | Candidate PR merged with Codex primary review `VERDICT: PASS` / `BLOCKERS: None` and status sync; Hermes patches PR fields after creation | Open — candidate PR pending |
...
That docs-only design gate has now been used: the Phase E persistent sessions / cancellation design packet is drafted on branch `docs/phase-e-persistent-sessions-cancellation-design` (candidate PR pending review/merge). The packet designs semantics only; it approves no implementation and no execution.

If/once the Phase E design PR merges, the next safe mainline request is a **separate local/offline implementation gate for the persistent-session lifecycle preflight / state machine** (injected fakes only; no real session launch; no cancellation execution) — not live/Gateway/Feishu/production config/real delivery, and not bounded real session execution:

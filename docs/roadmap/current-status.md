...
That separate gate has since been exercised for one narrow slice: the 2026-06-12 PRD → Claude architecture → Codex review → user review packet flow approved the **Phase C controlled local agent execution first implementation slice** (local/offline, default-off, `exec_controlled` one-shot only, pinned local acpx provenance, read-only Codex primary reviewer, atomic pre-launch claim/CAS). That slice merged in PR #114 (`21d1bafc22c6fcde2bb0af6fff6becfb0886cf4f`); the next allowed request on this mainline is:

```text
1. A separately approved Phase D deterministic real local smoke gate (pinned local acpx binary +

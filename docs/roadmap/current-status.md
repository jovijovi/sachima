...
| Live stream workbench/query integration | LS1-LS3 done; LS4 not approved | LS1 workbench composition, LS2 private source binding, and LS3 caller-API compatibility smoke are done as local/offline/default-off slices; LS4 remains a later separately approved task. |
...
1. **Request or approve a separately scoped LS4 runtime/query activation gate** — LS1 workbench composition, LS2 private source binding, and LS3 synthetic-artifact caller-API compatibility smoke are done as local/offline/default-off slices.
2. **Keep LS4 separate from live/default-on rollout** — later runtime/query activation still needs its own named approval and must not imply real AGENT/acpx/npx launch, Gateway/Feishu/live/default-on behavior, real delivery, service lifecycle, or production config.

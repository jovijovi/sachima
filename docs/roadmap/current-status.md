...
| Implementation — WP3a cancellation request-state + supervisor interrupt API | **PR candidate** — branch `feature/ars-wp3a-cancellation-interrupt-api`; local gates and final Codex blocker-only re-review PASS; not merged; PR/CI state must be checked live in GitHub after PR creation | Plan `docs/plans/2026-06-15-agent-run-supervisor-sachima-wp3a-cancellation-request-state-interrupt-api.md` + manifest + dev log; code `sachima_supervisor/activity_session_lifecycle.py`; tests `tests/sachima_supervisor/test_activity_session_lifecycle.py`; local verification: focused interrupt/cancel 22 passed / 93 deselected, lifecycle+real-execution 164 passed, `tests/sachima_supervisor` 518 passed, ruff/compileall/`git diff --check`/secret-scan/forbidden-surface-scan clean, codegraph up to date | Local/offline **injected-fake interrupt API state machine only**: adds `SESSION_INTERRUPT_API_APPROVAL_TOKEN`, `SessionInterruptRequest`/`SessionInterruptOutcome`, `SessionLifecycleStore._interrupt_idem`, the `cancel_interrupting`/`cancelled`/`cancel_failed`/`cancel_ambiguous` statuses, and `apply_session_interrupt(request, *, store, apply_interrupt)`. Requires a matching prior `cancel_requested` record for the same interrupt target (missing/mismatched => `activity_not_found` before fake, zero fake calls); records `cancelled` only when `interrupted=True` and `cleanup_verified=True`; `interrupted=False` => `cancel_failed`/`activity_supervisor_failed`; unverified cleanup / fake exception / unsafe / ambiguous outcome => `cancel_ambiguous`/`activity_cancel_ambiguous` held fail-closed. Performs **no real interrupt** and **no real cancellation execution**; `execute_real_cancellation` still rejects `activity_cancel_not_approved`; no live/Gateway/Feishu/production config/real delivery. WP3b bounded real cancellation execution remains separately gated |
...
> WP3a cancellation request-state + supervisor interrupt API is a **PR candidate** at doc-writing time (branch `feature/ars-wp3a-cancellation-interrupt-api`; local gates and final Codex blocker-only re-review PASS; not merged). PR number, URL, CI, and merge state are GitHub-live facts; this PR-tracking table should be refreshed after the PR exists or after merge/status-sync. See the WP3a phase-map row and canonical references above.
...
| ROADMAP-NEXT-ARS-WP3A-CANCELLATION-INTERRUPT-API | NEXT_PHASE | WP3a cancellation request-state + supervisor interrupt API is a **PR candidate** (branch `feature/ars-wp3a-cancellation-interrupt-api`; local gates and final Codex blocker-only re-review PASS; not merged; PR/CI state must be checked live in GitHub after PR creation). Local/offline injected-fake interrupt API state machine only: `apply_session_interrupt(...)` advances a prior `cancel_requested` record to `cancelled`/`cancel_failed`/`cancel_ambiguous`; no real interrupt, no real cancellation execution; `execute_real_cancellation` still rejects `activity_cancel_not_approved`. Worker history: Claude Code (Opus/max) architecture packet; Claude Code RED tests then stalled; Codex CLI substitute for GREEN + one narrow fix; Codex primary review round 1 BLOCKED stale in-flight replay finalization and missing prior-cancel target binding; two blocker regressions added RED first and fixed GREEN; Codex blocker-only review round 1/2 blockers fixed; final blocker-only re-review PASS. | No | No | Commit/push/PR/CI before any merge or status closure claim | Local verification (focused 22/93, lifecycle+real-execution 164, `tests/sachima_supervisor` 518, ruff/compileall/`git diff --check`/secret-scan/forbidden-surface-scan clean, codegraph up to date); plan `docs/plans/2026-06-15-agent-run-supervisor-sachima-wp3a-cancellation-request-state-interrupt-api.md` + manifest + dev log | Open — PR candidate; not merged |
| ROADMAP-NEXT-ARS-WP3B-REAL-CANCELLATION-EXECUTION | NEXT_PHASE | WP3b bounded real cancellation execution (turning an approved interrupt into a real stop of a running session) remains a separate later gate. WP3a's injected-fake interrupt API state machine does not approve it. | No | Yes | Any real interrupt or real cancellation execution claim | A new separate design/approval gate after WP3a is reviewed and merged; no live/Gateway/Feishu/production config/real delivery by default | Open |

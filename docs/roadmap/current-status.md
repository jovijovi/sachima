...
latest_phase_e_persistent_session_lifecycle_implementation_candidate: agent-run-supervisor × Sachima Phase E persistent session lifecycle preflight / state-machine implementation candidate (open branch; local/offline only; injected fakes only; no real session launch; no cancellation execution; no PR/merge/CI/commit claimed)
latest_phase_e_persistent_session_lifecycle_implementation_candidate_doc: docs/plans/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-session-lifecycle-state-machine-implementation.md
latest_phase_e_persistent_session_lifecycle_implementation_candidate_manifest: docs/plans/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-session-lifecycle-state-machine-implementation-manifest.yaml
latest_phase_e_persistent_session_lifecycle_implementation_candidate_dev_log: docs/dev_log/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-session-lifecycle-state-machine-implementation.md
...
current_position: P4 Sachima-side local conformance implemented; agentic-ui/cross-repo conformance pending as a side tail; agent-run-supervisor → Sachima local/offline integration, supervised local Activity, controlled dry-run evidence, durable-state preflight, and Phase C controlled local exec wrapper have merged; Phase D readiness docs merged in PR #117 and repo-side smoke prerequisites merged in PR #119; after separate user approvals, host-local DoR provisioning passed and one deterministic Phase D real local read-only Codex smoke ran through pinned local acpx 0.10.0 with execution pipeline PASS and business verdict PASS. Evidence is stored outside the repo under `/home/ubuntu/workspace/hermes/outputs/sachima/phase-d-real-local-smoke/`. This proves one bounded local read-only smoke only; persistent session execution, cancellation execution, write-capable roles, Satine/Hermes-profile ACP, controlled AI FLOW execution, live/public ingress, Gateway/Feishu involvement, production config writes, and real delivery remain unapproved. The Phase E persistent sessions / cancellation docs-only design packet merged in PR #123; the packet designs persistent-session and cancellation semantics only and does not approve implementation or any execution. A Phase E persistent-session lifecycle preflight / state-machine implementation candidate is open on branch `feature/phase-e-persistent-session-lifecycle`; it is local/offline only with injected fakes, no real session launch, no cancellation execution, and no PR/merge/CI/commit claimed.
implementation_marker_note: no live / no Gateway / no real delivery / Phase D real local read-only smoke PASS only / controlled one-shot exec wrapper + smoke prerequisites preparation + single smoke evidence
```
...
- agent-run-supervisor Sachima Phase E persistent-session lifecycle preflight / state-machine implementation candidate (open branch; no PR/merge/CI/commit claimed): `docs/plans/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-session-lifecycle-state-machine-implementation.md` (+ manifest `docs/plans/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-session-lifecycle-state-machine-implementation-manifest.yaml`)
- agent-run-supervisor Sachima Phase E persistent-session lifecycle preflight / state-machine implementation candidate dev log: `docs/dev_log/2026-06-13-agent-run-supervisor-sachima-phase-e-persistent-session-lifecycle-state-machine-implementation.md`
...
| ROADMAP-NEXT-ARS-PHASE-E-LIFECYCLE-STATE-MACHINE-IMPL | NEXT_PHASE | Phase E persistent-session lifecycle preflight / state-machine implementation candidate is open on branch `feature/phase-e-persistent-session-lifecycle`. This is the allowed local/offline Option A slice with injected fakes only; no real session launch, no cancellation execution, and no PR/merge/CI/commit claimed. | No | Yes | Any bounded real persistent-session implementation, cancellation execution, real AGENT/acpx execution, controlled AI FLOW, or live/Gateway/Feishu/production/delivery expansion | Fresh independent review, local verification, PR/CI/merge/status closure by repo owner; candidate must remain injected-fakes-only and no-leak/fail-closed | Open candidate branch |

## Explicit non-approvals

The current state does not approve any of the following outside the approved local/offline supervisor seam, the supervised local Activity first slice, the Phase C controlled local exec wrapper slice, and the single approved Phase D read-only real local smoke:
...
That docs-only design gate has now been used: the Phase E persistent sessions / cancellation design packet merged in PR #123. The packet designs semantics only; it approves no implementation and no execution.

After the Phase E design PR merged, the next safe mainline request became a **separate local/offline implementation gate for the persistent-session lifecycle preflight / state machine** (injected fakes only; no real session launch; no cancellation execution) — not live/Gateway/Feishu/production config/real delivery, and not bounded real session execution:
...
```text
additional real local smoke or real AGENT execution
persistent session execution / bounded real session launch

...
1. **Finish PR5 single-user production-shaped E2E merge gates** — focused/regression tests, no-leak/forbidden-surface scan, independent read-only blocker review, CI, and head-guarded merge; no real ingress/delivery/production config.
2. **After PR5 merges, re-baseline the production-launch gap** — summarize what the 5-PR mainline proves and what still needs separate approval for any live/default-on/runtime/production-facing work.

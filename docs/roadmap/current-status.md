...
| Current boundary | R1 runtime-spine core is local/offline only. This status page does not approve further runtime/source implementation, live/default-on behavior, real delivery, production config, Gateway lifecycle, Temporal Worker/service startup, real agent/acpx/npx execution, or write-capable agent roles. |
...
| R2 supervisor execution port implementation | Not approved | Any local/offline execution-port source implementation for supervised external AGENT event-stream semantics. |
...
1. **Request the R2 supervisor execution port implementation gate** — source/runtime work for R2 is not approved by this status page; use the R2 approval text in `docs/plans/2026-07-03-sachima-private-hermes-runtime-spine-development-plan.md`.
...
- R2-R5 runtime-spine source/runtime implementation without the matching named gate;

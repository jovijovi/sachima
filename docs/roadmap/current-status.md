...
| Current boundary | R1/R2/R3/R4/R5 runtime-spine work and the existing agent-run-supervisor foundation remain local/offline/default-off. The live-stream integration plan may advance only through separately approved governed slices: LS1 local/offline workbench composition, LS2 private source binding, LS3 synthetic-artifact caller-API compatibility smoke, and later LS4 runtime/query activation gate. This status page still does not approve live/default-on behavior, real delivery, production config, Gateway lifecycle, Temporal Worker/service startup, write-capable agent roles, public webhook exposure, real external Sachima ingress, or broad real acpx/npx/agent execution. |
...
| Agent-run-supervisor integration foundation | Done | Adapter, host-local smoke shape, persistent lifecycle, workbench view, production-shaped E2E, and live-progress safe projection are complete as local/offline/default-off foundation. Fresh repo/runtime checks are still required before reuse. |
| Live stream workbench/query integration | In progress under named approval | LS1 workbench composition and LS2 private source binding are done; LS3 is the current in-progress implementation task, and LS4 remains a later separately approved task. |
...
| Real agent / acpx / npx execution | Not approved for LS1/LS2/LS3 except synthetic-artifact caller-API import smoke | LS1 and LS2 use fakes/local binding only; LS3 may import/read synthetic artifacts through the caller API but must not launch a real agent/acpx/npx process. Any broader real execution remains not approved. |
...
1. **LS3 synthetic-artifact caller-API compatibility smoke — the current in-progress separately approved slice** — verify the imported/read-only caller API against synthetic artifacts only, without real AGENT/acpx/npx launch, Gateway/Feishu/live/default-on behavior, real delivery, service lifecycle, or production config. LS1 workbench composition and LS2 private source binding are done.
2. **Then proceed only through a separately approved LS4 gate** — later runtime/query activation remains separately gated and needs its own named approval.

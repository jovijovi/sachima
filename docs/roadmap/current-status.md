...
latest_controlled_local_exec_implementation: agent-run-supervisor × Sachima controlled local agent execution first slice (Phase C; exec_controlled one-shot wrapper; local/offline only; default-off; implemented in worktree feature/ars-controlled-local-exec, Codex re-review PASS, awaiting PR; real local smoke NOT run)
...
| ROADMAP-NEXT-ARS-CTRL-EXEC-REVIEW-PR | NEXT_PHASE | Phase C controlled local exec first slice is implemented in worktree `feature/ars-controlled-local-exec`; the first Codex primary review returned VERDICT: BLOCK on the unlocked claim-store check-and-set, fixed in-worktree (locked in-process CAS + true concurrent tests), and Codex blocker re-review returned `VERDICT: PASS` / `BLOCKERS: None`. | No | Yes | Any claim that Phase C is complete/merged | Hermes-owned PR/CI/merge and this file updated with the merge evidence | Open |
| ROADMAP-NEXT-ARS-CTRL-EXEC-REAL-SMOKE | NEXT_PHASE | Phase D deterministic real local smoke (read-only Codex one-shot) is currently BLOCKED: no pinned local acpx binary exists on this host (committed role config truthfully keeps `acpx_binary: null` and the provenance gate fails closed), and prompt materialization is deliberately unimplemented in the Phase C slice. | No | Yes | Any real local smoke or real AGENT execution claim | Operator pins a verified local acpx executable in the role config, a separately approved prompt-materialization gate lands, and the smoke runs with no Gateway/delivery/public ingress and sanitized evidence only | Open — BLOCKED |

## Explicit non-approvals

The current state does not approve any of the following outside the approved local/offline supervisor seam, the supervised local Activity first slice, and the Phase C controlled local exec wrapper slice (which itself ran no real agent and remains unmerged):
...
That separate gate has since been exercised for one narrow slice: the 2026-06-12 PRD → Claude architecture → Codex review → user review packet flow approved the **Phase C controlled local agent execution first implementation slice** (local/offline, default-off, `exec_controlled` one-shot only, pinned local acpx provenance, read-only Codex primary reviewer, atomic pre-launch claim/CAS). That slice is implemented in worktree `feature/ars-controlled-local-exec` (see the Phase C row above) and is **not** merged; the next allowed requests on this mainline are, in order:

```text
1. Hermes-owned PR / CI / merge and status closure in this file
2. A separately approved Phase D deterministic real local smoke gate (pinned local acpx binary +
   prompt materialization approval) — currently BLOCKED, see ROADMAP-NEXT-ARS-CTRL-EXEC-REAL-SMOKE
```
...
- The durable-state preflight implementation candidate validates durable preconditions and stores sanitized query state only; it does not approve real local `exec`, persistent sessions, cancellation execution, real AGENT execution, controlled AI FLOW execution, a Gateway-owned or Worker-owned runtime lifecycle, live/default-on behavior, Gateway involvement, real ingress, or real delivery.

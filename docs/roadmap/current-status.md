...
latest_phase_d_smoke_prerequisites_implementation: agent-run-supervisor × Sachima Phase D smoke prerequisites implementation (pinned local acpx provenance verifier with injected probe; deterministic prompt materialization fixture/builder + explicit prompt_materializer seam defaulting to Phase C prompt=None; agent_run_supervisor exact-pin checker; local/offline, injected fakes only; no smoke run; prepared on branch feat/phase-d-smoke-prerequisites; Codex blocker-only re-review PASS; PR pending)
...
The Phase D readiness gate is now merged docs-only. Claude Code attempted architecture/docs for the readiness gate and then hit session-limit / 429; Codex CLI substituted for the architecture/docs authoring role. Fresh-context Codex primary review first returned BLOCK on two docs consistency issues; after narrow fixes, blocker-only re-review returned PASS / BLOCKERS None. The next allowed request on this mainline is **Definition-of-Ready provisioning / implementation planning for Phase D smoke**, not smoke execution:

```text
1. Separately approved preparation for Phase D smoke prerequisites: verified pinned local acpx,
   installed/pinned agent_run_supervisor, and prompt-materialization implementation — still with no
   Gateway/Feishu/live, no production config write, and no smoke until a later explicit approval.
```

That prerequisites-preparation request has now been exercised: the 2026-06-12 user approval ("批准准备 Phase D smoke prerequisites：实现并验证 pinned local acpx provenance、prompt materialization、agent_run_supervisor 安装/固定方案…") produced the prerequisites implementation slice on branch `feat/phase-d-smoke-prerequisites` (plan/manifest/dev log under `docs/plans/2026-06-12-agent-run-supervisor-sachima-phase-d-smoke-prerequisites-*`), with Codex blocker-only re-review PASS / BLOCKERS None; PR and CI still pending. The slice ran no smoke and launched no agent.

After that review and user approval, a **later** Phase D deterministic real local smoke request remains BLOCKED until the operator provides a verified pinned local `acpx` executable via an untracked local-only role overlay, the `agent_run_supervisor` library is installed/pinned on the host, the prerequisites slice is merged, and the user gives a separate named smoke approval. See `ROADMAP-NEXT-ARS-CTRL-EXEC-REAL-SMOKE`.

...
latest_supervised_local_activity_durable_state_preflight_implementation: agent-run-supervisor × Sachima supervised local Activity durable-state preflight implementation (local/offline only; implementation candidate in PR #107)
...
| #107 | agent-run-supervisor supervised local Activity durable-state preflight implementation | TBD | Local/offline preflight candidate only; validates durable preconditions and stores sanitized query state; no real local exec/sessions/cancellation execution/real AGENT execution/controlled AI FLOW/live/Gateway/real delivery approval |

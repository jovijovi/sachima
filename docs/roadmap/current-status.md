...
| Current phase | S3 hermetic-local Temporal Activity implementation — implementation candidate on the feature branch. It implements the Activity-compatible body and caller-owned controller that call the merged S2 local/offline supervisor-adapter seam. The S1 design packet, S2 seam, and S3 design packet are merged. |
...
| Current implementation focus | S3 hermetic-local Activity/controller implementation: `ActivityInput -> S2LocalOfflineSupervisorAdapter -> ActivityOutput`, caller-owned start/query/update/recover/retry/close controller, closed intent-class→role-key mapping, stable claim-check/evidence refs and stable codes, duplicate/recover/no-relaunch handling, SCAN 1/SCAN 2 no-leak gates, and ops-owned Worker/task-queue boundary. Still injected-fake and controlled-deterministic only: no real agent/acpx/npx run, no Gateway/Feishu/live/default-on/public-ingress behavior, no real send, no production config writes, and no write-capable roles. |
...
| Not yet started | The later slices (S4 read-only real-agent step, S5 downstream delivery reconnect); the paused P7 bounded real-send canary execute; limited live pilot; and P8 product/ops hardening. |
...
1. **Review/merge S3 hermetic-local Temporal Activity implementation** — run local verification, Codex blocker review, GitHub CI, and merge approval for the current implementation candidate. Scope remains injected-fake / controlled-deterministic only: no real agent/acpx/npx, no Gateway/Feishu/live/default-on behavior, no real send, no production config, and no write-capable roles.
2. **S4 read-only real-agent step design/implementation gate** — only after S3 is merged and separately approved; this would connect agent-run-supervisor real read-only execution, still without delivery/live expansion.
...
The current S3 implementation candidate is hermetic-local and injected-fake only: it may exercise local/unit/hermetic Temporal verification, but it does not approve real agent/acpx/npx execution, Gateway/Feishu/live/default-on behavior, real send, production config, write-capable roles, production cluster/traffic, or Gateway-owned Worker/service/subprocess lifecycle. Later integration slices (S4–S5) each require their own named approval before any real-agent step or delivery reconnection.

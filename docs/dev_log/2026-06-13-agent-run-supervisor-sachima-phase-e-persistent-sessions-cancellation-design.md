...
PR: candidate — not yet opened (Hermes patches PR number/merge fields after creation)
...
manifest YAML parse + required keys (status: design_candidate_pr_pending; docs_only: true; all execution/live/production/delivery booleans false)
status sync check against docs/roadmap/current-status.md (candidate wording; no invented PR number)
...
If/once this candidate design PR merges, the next safe request is the **Option A** implementation gate only:

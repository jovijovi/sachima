...
| **MiniMax-CN TTS** | Excellent | Paid | `MINIMAX_CN_API_KEY` |
...
tts:
  provider: "edge"              # "edge" | "elevenlabs" | "openai" | "minimax" | "minimax-cn" | "mistral" | "gemini" | "xai" | "neutts" | "kittentts" | "piper"
  ...
  minimax:
    ...
    vol: 1                      # 0 - 10
    pitch: 0                    # -12 - 12
  minimax-cn:                   # MiniMax China service — separate account from global minimax
    # Auth: MINIMAX_CN_API_KEY + MINIMAX_CN_GROUP_ID (never borrows the global MINIMAX_* values)
    model: "speech-2.8-hd"
    voice_id: "English_Graceful_Lady"  # See https://platform.minimaxi.com
    speed: 1                    # 0.5 - 2.0
    vol: 1                      # 0 - 10
    pitch: 0                    # -12 - 12
    # base_url: "https://api.minimaxi.com/v1/t2a_v2"   # default CN endpoint
  mistral:
...
- **MiniMax TTS** and **MiniMax-CN TTS** output MP3 and need **ffmpeg** to convert for Telegram voice bubbles
...
Without ffmpeg, Edge TTS, MiniMax TTS, MiniMax-CN TTS, NeuTTS, KittenTTS, and Piper audio are sent as regular audio files (playable, but shown as a rectangular player instead of a voice bubble).

...
This bundled plugin normalizes Sachima Envelope v1 webhook payloads into
Hermes gateway events and sends replies through a configured callback URL.
The in-memory sender is available only through explicit test mode.
"""
...
def _scoped_secret(name: str) -> str:
    """Read a profile-owned credential, with default-profile compatibility."""
    try:
...
def validate_config(config: PlatformConfig) -> bool:
    """Require real callback egress unless explicit local test mode is set."""
    extra = config.extra or {}
    return bool(
        extra.get("local_mode")
        or extra.get("delivery_url")
        or extra.get("send_url")
    )


def is_connected(config: PlatformConfig) -> bool:
    """Report readiness from the same minimum config used at startup."""
...
class SachimaAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        self._wire_plugin_handlers(self._web_app)
    ...
    def _validate_webhook_signature(
        ...
        secret = str(
            (self.config.extra or {}).get("webhook_secret")
            or _scoped_secret("SACHIMA_WEBHOOK_SECRET")
        ).strip()
        if not secret:
            return None
    ...
    async def _extract_url_image_media(self, payload: Dict[str, Any]) -> tuple[list[str], list[str]]:
        ...
        for attachment in self._image_attachments_from_payload(payload):
            ...
            ext = self._image_extension_for_mime(mime_type)
            try:
                # The upstream shared cache performs URL safety checks and
                # enforces gateway.max_inbound_media_bytes while streaming.
                media_urls.append(await cache_image_from_url(str(url), ext))
            except ValueError as exc:
                raise SachimaPayloadError(str(exc)) from exc
            media_types.append(mime_type)
        return media_urls, media_types
    ...
    def _signed_delivery_headers(self, raw_body: bytes) -> dict[str, str]:
        ...
        api_key = str(
            self.config.api_key
            or (self.config.extra or {}).get("api_key")
            or _scoped_secret("SACHIMA_API_KEY")
        ).strip()
        if api_key:
    ...
    async def send(
        ...
        if not delivery_url and (self.config.extra or {}).get("local_mode") is True:
        ...
        if not delivery_url:
            return SendResult(
                success=False,
                error=(
                    "Sachima delivery_url is not configured; local recording "
                    "is available only with platforms.sachima.extra.local_mode=true"
                ),
            )
...
def register(ctx) -> None:
    """Register the default-off Sachima channel through the plugin seam."""
    ctx.register_platform(
        name="sachima",
        label="Sachima",
        adapter_factory=lambda config: SachimaAdapter(config),
        check_fn=check_sachima_requirements,
        ...
        platform_hint=(
            "You are chatting in the Sachima text IM channel. Keep replies "
            "concise and use plain text unless the user asks for formatting."
        ),
    )

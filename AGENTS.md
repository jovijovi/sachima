...
Short version: Sachima should become a production-grade AI workbench inside a custom IM channel: a safe, durable, observable, and recoverable Hermes/FlowWeaver system that can receive real IM requests, orchestrate long AI workflows, deliver results back through the channel, and preserve clear operational control.

Planning references:

- `GOAL.md` — final goal, target architecture, and non-negotiable principles.
- `docs/roadmap/current-status.md` — lean current project dashboard: phase, feature/task status, blockers, explicit non-approvals, and next allowed work. Its `Machine-owned dynamic status` block is generated by `tools/sync_roadmap_status.py`; treat that block as git/GitHub evidence only, not as phase approval.
...
For any Sachima / FlowWeaver roadmap, phase-gate, PR, CI, review, merge, or next-phase-readiness work, the agent must read `docs/roadmap/current-status.md` and the specific plan/runbook/source files relevant to the requested task.

Before making changes, the agent must identify:
...
If `docs/roadmap/current-status.md` is missing, stale, or contradicts live repo/GitHub truth, stop and report the drift risk before making changes.
...
- the feature/task row in `docs/roadmap/current-status.md` reflects the new state, or the PR explains why the status update is not applicable;
- blockers and explicit non-approvals remain accurate.

Do not use `current-status.md` as a PR/merge log. GitHub is the authority for PR, commit, CI, and merge history.

### Phase Gate Drift Control

For Sachima multi-phase, production-adjacent, high-risk, or next-phase-readiness work, apply the roadmap governance workflow in this section before:
